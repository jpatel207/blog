...
class User(UserMixin, db.Model):
    ...
    #adds a new column of author in the child
...
class BlogPost(db.Model):
    ...
    #name of class in lower_case.id
...
def register():
    ...
    if request.method == 'POST':
        pshash = generate_password_hash(form.password.data, 
                                        method='pbkdf2:sha256', salt_length=8)
        matched_user = User.query.filter_by(email=form.email.data).first()
        if matched_user:
            flash('User account already exists. Please log-in with your email.')
...
def login():
    ...
    if request.method == 'POST':
        matched_user = User.query.filter_by(email=form.email.data).first()
        if matched_user:
            check_password = check_password_hash(matched_user.password, 
                                                    form.password.data)
            if check_password:
